...
function harness(saved = null) {
  ...
  const exam = { id: 'test', version: '1', title: 'Test', durationMinutes: 1, questions: [{ answer: [1], points: 10, forbidden: [] }], passScore: 10, forbiddenLimit: 1 };
  ...
  return { nodes, exam, setTime: value => { now = value; }, get writes() { return writes; }, get stored() { return stored && JSON.parse(stored); } };
}
